...
export const RunSummaryOverlay: React.FC<RunSummaryOverlayProps> = ({ stats, purchaseLog, onContinue }) => {
  ...
  const buildText = useCallback(() => {
    ...
    lines.push(`Flame Burst: ${t?.flameBurstUses ?? 0} uses | Dmg to Enemies: ${t?.flameBurstDamageToEnemies ?? 0} | Dmg to Gate: ${t?.flameBurstDamageToGate ?? 0} | Passive: ${t?.flamePassiveDamageDealt ?? 0}`);
    if ((t?.flameUnlockedAt ?? -1) >= 0) lines.push(`  Flame unlocked at: ${fmt(t?.flameUnlockedAt ?? 0)}s`);
    if (t?.flameBurstTimestamps && t.flameBurstTimestamps.length > 0) lines.push(`  Burst timestamps: ${t.flameBurstTimestamps.map(ts => fmt(ts, 1)).join(', ')}`);
    lines.push('');
    ...
    lines.push(`Bomb Uses: ${t?.tonicBombUses ?? 0} | Star Throws: ${t?.starThrowUses ?? 0} | Flame Bursts: ${t?.flameBurstUses ?? 0}`);
    lines.push('');
